...
图论与深度优先搜索的结合,本质是一个深度优先搜索题目，没有充分的图论只是也能解决，不过一定的图论只是对于解题思路理解很有帮助。
```java
// 节点key着的色value
...
<br><br>两个点集合 S, U。S表示距离源点最短路径长度已确定的点集合，U表示距离源点最短路径长度未确定的点集合。
<br><br>初始化:设置最短路径数组`int[] dist`, dist[i]表示`源点到节点i` 的最短路径长度，假设源点为`A`，则dist[A]=0，其他元素为+∞。
将A节点加入到S集合中，将A节点的其他相邻点加入到U集合中，并更新这些相邻点到源点的最小路径长度dist[x]。
<br><br>每次从U种取出距离源点路径长度最小的点，假设为M，将其加入到S集合，
且 **尝试去** **更新** M在集合U中的相邻点 距离源点的最小长度。直至U集合为空，否则重复当前步骤。
![](迪克特斯拉算法.png)
