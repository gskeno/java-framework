...
public class 网络延迟时间 {
    ...
    public int networkDelayTime1(int[][] times, int n, int k) {
        ...
        // 元素类型为int[], 表示的是一个节点，该节点到源点的最短距离为int[0], 该节点的id为int[1]
        PriorityQueue<int[]> queue = new PriorityQueue<>((o1, o2) -> {
            // 以节点到源点的最短路径长度做最小堆
            return o1[0] != o1[1] ? o1[0] - o2[0] : o1[1] - o2[1];
        });
        ...
        while (!queue.isEmpty()){
            ...
            for(int[] neighbor : table[x]){
                ...
                if (dist[x] + neighborDist < dist[neighborId]){
                    dist[neighborId] = dist[x] + neighborDist;
                    // 这里可能导致同一个节点多次进队列
    ...
    public int networkDelayTime2(int[][] times, int n, int k) {
        ...
        while (!queue.isEmpty()){
            ...
            // x节点的相邻节点
            int[] neighbors = grid[x];
            for (int i = 0; i < neighbors.length; i++) {
                // x节点有效的 相邻节点
                if (i != x && neighbors[i] != INF){
                    int neighborId = i;
                    int neighborDist = neighbors[i];
                    if (nearDist + neighborDist < dist[neighborId]){
                        // 源点到neighborId节点的最短距离确定，且不会改变, 且可能导致同一个节点多次进队列
...
//        网络延迟时间 solution = new 网络延迟时间();
//        int ans = solution.networkDelayTime2(new int[][]{
//                {1,2,1}
//        }, 2, 2);
//        System.out.println(ans);
        网络延迟时间 solution = new 网络延迟时间();
