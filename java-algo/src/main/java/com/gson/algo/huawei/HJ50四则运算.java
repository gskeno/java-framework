...
public class HJ50四则运算 {
     ...
     *    a. 每遍历到一个字符
     *       a.1 如果是数字，则push入数字栈
     *       a.2 如果是-, 表示负号，则将符号和后续连在一起的数字组成一个负数push到数字栈A中
     *       a.3 如果是-, 表示减号，则执行a.4
     *       a.4 如果是运算符
     *              a.2.1 比栈顶符号优先级低或相等，则执行函数f,再入符号栈B 。如 5 + 2 - 3中遍历到-时,
     *              a.2.2 不满足a.2.1 条件时，则将运算符入符号栈B, 如 2 *[3+2*5] 遍历到+号时。
     *              举例: 我们只认为 +,-比 +,-,*,/优先级低或者相等； *,/比 *,/优先级低或者相等。此时可以执行f函数了
     *       a.5 如果是左括号，则直接入栈
     *       a.6 如果是右括号，则一直执行函数f，直至遇到符号栈B顶部元素是左括号停止，且将左括号弹栈
     *
     *    b. 字符遍历完之后，如果符号栈
     *
     *
