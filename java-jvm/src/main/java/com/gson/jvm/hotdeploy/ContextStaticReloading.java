package com.gson.jvm.hotdeploy;
...
public class ContextStaticReloading {
    ...
    public static Object newContext(Cache cache)
        ...
        String className = "com.gson.jvm.hotdeploy.ContextStaticReloading$Context";
        Class<?> contextClass = new MyClassLoader("java-jvm/target/classes").loadClass(className);
        Object context = contextClass.newInstance();
        // //初始化之前显把context对象里面的cache进行赋值，这里要特别注意的是，我们传入的cache对象是由系统类加载器加载的。
        // 而Context类里面的Cache类却是由自定义类加载器MyClassLoader加载，
        // 这样赋值会造成类型和值不一致，导致抛出IllegalArgumentException异常。
        //
        // 解决思路很简单，让他们保持一致就行，但是由于我们Cache必须又系统默认类加载器加载，
        // 那么只能让Context里面的Cache显性的指定系统类加载器来加载(
        // 因为不指定的话，根据类加载器依赖传导原则，依赖类复用被依赖的类的加载器)。
        //
        // 本例子很简单，就是在复写loadClass方法的时候，增加条件!name.contains("Cache")，
        // 让MyClassLoader加载Cache时抛给系统类加载器。
        contextClass.getDeclaredField("cache").set(context,cache);
